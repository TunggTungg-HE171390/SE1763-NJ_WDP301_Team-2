...
const cancelScheduleByPatient = async (req, res, next) => {
    try {
        ...
        statusMessage = `🔴 Cuộc hẹn của bạn với bác sĩ ${appointment.psychologistId.fullName} vào lúc [Thời gian cuộc hẹn] ngày [Ngày] đã bị hủy.`
        await mailService.sendEmail(appointment.patientId.email, appointment.psychologistId.fullName, statusMessage, actions.CANCEL_APPOINTMENT);
