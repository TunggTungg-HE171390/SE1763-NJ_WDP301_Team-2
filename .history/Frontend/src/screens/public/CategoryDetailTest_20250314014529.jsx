...
export function CategoryDetailTest() {
    ...
    useEffect(() => {
        const fetchTestData = async () => {
            try {
                // Gọi API lấy dữ liệu bài kiểm tra theo categoryId
                const response = await getTestByCateId(categoryId);
                
                // Kiểm tra xem response có chứa dữ liệu trong response.data không
                if (response && response.data) {
                    console.log("Fetched test data:", response.data, response.data.questionCount);
                    setTestData(response.data); 
                    
                    console.log("Test data:", testData.map((test) => test._id));
                } else {
    ...
    //         try {
    //             const response = await getCountQuestionOnTheTest(testId);
