...
export function EditTestScreen() {
  ...
  const handleSaveTest = async () => {
    try {
      ...
      const questionUpdatePromises = questionsArray.map(q =>
        updateQuestion(q._id, { content: q.content, answers: q.answers })
      );
  ...
  // Thêm câu hỏi mới, giới hạn số lượng câu hỏi
  const addQuestion = () => {
    if (questionsArray.length < 10) { // Giới hạn tối đa 10 câu hỏi
      setQuestionsArray([...questionsArray, { content: "", answers: [{ content: "Đáp án 1", point: 0 }] }]);
    } else {
      alert("Tối đa 10 câu hỏi cho bài kiểm tra");
    }
  };

  // Thêm đáp án mới, giới hạn tối đa 6 đáp án cho mỗi câu hỏi
