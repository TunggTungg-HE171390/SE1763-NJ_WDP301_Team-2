...
export function EditTestScreen() {
    ...
    const handleSaveTest = async () => {
        try {
            console.log("questionsArray", title, description, testOutcomes);
            console.log("questionsArray", typeof questionsArray);
            const testUpdateResponse = await updateTest(testId, {
                title: title,              // Chỉ truyền chuỗi cho title
                description: description,  // Chỉ truyền chuỗi cho description
                testOutcomes: testOutcomes // Giữ nguyên cấu trúc testOutcomes
              });

            console.log("Test update response:", testUpdateResponse);

            // 2. Cập nhật các câu hỏi sau khi bài kiểm tra đã được cập nhật
