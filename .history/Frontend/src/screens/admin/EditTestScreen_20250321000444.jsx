...
export function EditTestScreen() {
    ...
    const deleteQuestion = (index) => {
        ...
        const deleteResponedeleteRespone = await updateTest(testId, title, description, testOutcomes); 
        setQuestionsArray(updatedQuestions);
    };
                ...
                <CardContent className="grid gap-6">
                    {questionsArray.map((question, questionIndex) => (
                        <div key={questionIndex} className="border p-4 mb-4 rounded">
                            ...
                            <Button onClick={() => deleteQuestion(question.questionId)}>Xóa câu hỏi</Button>
                        </div>
                    ))}
                </CardContent>
