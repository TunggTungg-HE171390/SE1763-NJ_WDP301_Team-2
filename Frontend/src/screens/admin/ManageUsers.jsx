...
const UserForm = ({ user, onChange, onSubmit }) => {
  ...
  return (
    <Form onSubmit={onSubmit}>
      ...
      <Form.Group controlId="password" hidden={isEdit}>
...
const ManageUsers = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const response = await fetch(`http://localhost:${import.meta.env.VITE_PORT}/api/admin/allaccount`, {
          headers: {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`http://localhost:${import.meta.env.VITE_PORT}/api/admin/deleteaccount/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
        },
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const method = newUser._id ? 'PUT' : 'POST';
      const url = newUser._id
        ? `http://localhost:${import.meta.env.VITE_PORT}/api/admin/updateaccount/${newUser._id}`
        : `http://localhost:${import.meta.env.VITE_PORT}/api/admin/addaccount`;
      ...
      const data = await response.json();
      if (newUser._id) {
        setUsers(users.map(user => user._id === newUser._id ? data : user));
    ...
    <div className="container mt-5">
      ...
      <Modal show={showModal} onHide={handleClose}>
        <Modal.Header closeButton>
          <Modal.Title>{newUser._id ? 'Edit User' : 'Add User'}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
